...
| `alias` |Меняет обычные команды на команду, которая тебе нравиться|`alias "l"="ls"`|
| `ssh` | Удаленное подключение к серверу|`ssh-keygen -t rsa -C "you_email@exemple.com"`|
| `#` |Комментарий|`#SODA-IO`|
| `&` |Запустить процесс в фоновом режиме|`sleep 30 &`|
| `cat`|Объединение `данных` в один `File`|`cat вывод содержимого файла`|
...
|`head` |Выводит начальную часть файла| `head файл.txt`| 
